# main.py — AguaRuta Backend (versión definitiva)
# Fecha: 2025-09-30
...
DATA_MODE = os.getenv("DATA_MODE", "db").lower().strip()  # db | excel
...
def sync_files_to_db() -> int:
    """Lee Excel/CSV (prioriza Excel) y pisa DB"""
...
# =============================================================================
# RUTAS ACTIVAS — CRUD + FILTROS + IMPORT/PREVIEW
# =============================================================================
...
# =============================================================================
# MAPA (solo registros con lat/lon) + color por camión
# =============================================================================
@app.get("/mapa-puntos")
def mapa_puntos(camion: Optional[str] = None):
    if DATA_MODE == "db":
        conn = db_conn(); cur = conn.cursor()
        q = """
            SELECT id, camion, nombre, litros_entrega AS litros, latitud, longitud
            FROM rutas_activas
            WHERE latitud IS NOT NULL AND longitud IS NOT NULL
        """
        params=[]
        if camion:
            q += " AND UPPER(camion)=UPPER(%s)"; params.append(camion)
        q += " ORDER BY camion, nombre"
        cur.execute(q, params)
        rows = cur.fetchall(); cur.close(); db_put(conn)
        cols = ["id","camion","nombre","litros","latitud","longitud"]
    ...
    for d in data:
        d["color"] = CAMION_COLORS.get(str(d["camion"]).upper(), "#1e40af")
        # popup simple: nombre + litros
        d["popup"] = f"{d.get('nombre','')} — {d.get('litros','')} L"
    return {"status": "ok", "data": data}

# =============================================================================
# ENTREGAS APP (foto + GPS) + NO ENTREGADAS + ESTADISTICAS
# =============================================================================
